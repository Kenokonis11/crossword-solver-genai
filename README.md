...
1. **Source a crossword PDF.** Go to the [LA Times Crossword](https://www.latimes.com/games#clx-item-1) (or any **PuzzleMe-powered** crossword platform), select a puzzle, and use the **print/PDF** option to save it.

   ![Where to find the print option on the LA Times crossword page](docs/Crossword_Print.jpg)

   > **Supported Platforms:** This parser is designed for crossword PDFs generated by the **PuzzleMe** (AmuseLabs) engine. The **LA Times Crossword** is the primary tested platform. Other PuzzleMe-powered outlets (e.g., *The Washington Post*, *USA Today*, *The New Yorker*) should also work, though your mileage may vary. PDFs from non-PuzzleMe sources may have different vector structures that produce lower accuracy.
...
Below is a screenshot of the tutor in action after uploading a crossword PDF. The sidebar shows the solver summary, and the chat demonstrates the hint escalation protocol:

![Sample conversation with the AI Crossword Tutor after uploading a PDF](docs/Crossword_Screenshot.jpg)
